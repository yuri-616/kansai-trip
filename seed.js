...
(function () {
  ...
  const c = (cat, text, important) => ({ id: id(), cat, text, done: false, important: !!important });
  const checklist = [
    c('證件金錢', '護照（效期 6 個月以上）', true),
    c('證件金錢', 'Visit Japan Web QR code（入境、海關）', true),
    c('證件金錢', '日幣現金'),
    c('證件金錢', '信用卡（最好兩張）'),
    c('證件金錢', '西瓜卡 Suica（可以加進 Apple 錢包，關西也能刷）'),
    c('證件金錢', '零錢包'),
    c('電子通訊', '網卡 / eSIM'),
    c('電子通訊', '行動電源（隨身帶，確認虎航規定）'),
    c('電子通訊', '充電線、充電頭（日本插座和台灣相同）'),
    c('電子通訊', '腳架'),
    c('衣物', '薄外套（京都早晚約 18°C）'),
    c('衣物', '好走的鞋（USJ、竹林會走很多）'),
    c('衣物', '雨衣雨傘（9 月底還是颱風季）'),
    c('衣物', '帽子'),
    c('衣物', '墨鏡'),
    c('盥洗藥品', '防曬'),
    ...
    c('盥洗藥品', '護墊、衛生棉'),
    c('盥洗藥品', '牙套、牙線'),
    c('盥洗藥品', '常備藥、暈車藥（保津川遊船）'),
    c('其他', '水壺'),
    c('其他', '環保袋'),
    c('其他', '素食說明：用 App 的「日語」分頁，點第一張卡給店員看'),
    c('其他', '隨身小點心（素食者在觀光區常常找不到能吃的）'),
    c('其他', '虎航行李重量（回程伴手禮要留空間）'),
    c('預約待辦', '確認 Ermine\'s Kitchen 預約時間（9/26）'),
    c('預約待辦', '湯豆腐 嵯峨野 訂位（9/26 晚餐，豆腐賣完會提早收）'),
    c('預約待辦', '雍州路：確認 9/28（週一）有開、要不要訂位', true),
    c('預約待辦', '和牛店（Kijurou 等）訂位'),
    c('預約待辦', 'hair salon Yoi 剪髮時間（9/29）'),
    c('預約待辦', '嵯峨野小火車（9/27 09:00）'),
    c('預約待辦', '保津川遊船（9/27 10:00）'),
    c('預約待辦', '祐斎亭'),
    c('預約待辦', '貴船川床料理（改到 9/28 午餐）'),
    c('預約待辦', 'USJ 門票 / 快速通關', true),
    c('預約待辦', '航海王劇場'),
    c('出發前待辦', '貓咪寄養確認'),
    c('出發前待辦', '買魚尾裙'),
    c('出發前待辦', '理髮'),
  ...
  // 每次更新 seed 內容就把這個號碼 +1：App 會把新增的清單項目自動補進使用者手機，不動他已改過的部分
  window.SEED_VERSION = 3;
  window.SEED = { version: 1, seedVersion: window.SEED_VERSION, rate: 0.21, days, checklist, expenses };
})();
